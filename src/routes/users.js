  ...
  try {
    ...
    if (req.body.isDonor) {
      ...
      if (userExists) {
        ...
        res.status(400).send('Este documento de identidad no existe, si quiere donar desmarque la casilla "SOY DONADOR"')
      }

    } else {
      if (userExists) {
        res.status(400).send('Este documento de indentidad ya existe, si quiere volver a donar marque la casilla "SOY DONADOR"');
      } else {
